...
class BatchRewardManager(AbstractRewardManager):
    ...
    def verify(self, data):
        ...
        for i in range(len(data)):
            valid_response_len = valid_response_lengths[i]
            valid_response_ids = response_ids[i][:valid_response_len]
            response_str = self.tokenizer.decode(valid_response_ids, skip_special_tokens=True)
            responses_str.append(response_str)

            valid_prompt_len = valid_prompt_lengths[i]
            valid_prompt_ids = response_ids[i][:valid_prompt_len]
            prompt_str = self.tokenizer.decode(valid_prompt_ids, skip_special_tokens=True)
            responses_str.append(prompt_str)
    ...
    def __call__(self, data: DataProto, return_dict: bool = False) -> torch.Tensor | dict[str, Any]:
        ...
        for i in range(len(data)):
            ...
            prompt_length = valid_prompt_lengths[i].item()
            reward = scores[i]
            ...
            if already_printed.get(data_source, 0) < self.num_examine:
                response_str = self.tokenizer.decode(data.batch["responses"][i][:response_length], skip_special_tokens=True)
                prompt_str = self.tokenizer.decode(data.batch["prompts"][i][:prompt_length], skip_special_tokens=True)
